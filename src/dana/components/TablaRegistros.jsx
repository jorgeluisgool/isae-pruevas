...
const TableRegistros = ({
  ...
  const handlePDF = (event, registro) => {
    ...
    fetch(`${api}/abrir/documento/inventario/${registro.idinventario}`, {
      ...
      .catch((error) => console.log(error));
    window.open(
      `${api}/abrir/documento/inventario/${registro.idinventario}`,
      "_blank"
    );
    //  }
  };
  ...
  const handleEvidences = (event, registro) => {
    ...
    fetch(
      `${api}/obtener/firmas/proyecto/${registro.proyecto.idproyecto}/${registro.idinventario}`,
    ...
    fetch(
      `${api}/obtener/campos/checkboxevidencia/proyecto/${registro.proyecto.idproyecto}/${registro.idinventario}`,
      { method: "GET", headers: { "Content-Type": "application/json" } }
    )
      .then((response) => response.json())
      .then((responseData) => {
    ...
    <>
              ...
              <td className="px-6">
                <div className="flex space-x-4">
                  <button
                    type="button"
                    onClick={(event) => {
                      handleEvidences(event, registro);
                    }}
                    className="w-14 h-14 object-cover active:scale-[.98] bg-transparent hover:bg-[#245A95] hover:text-white text-[#245A95] text-2xl font-bold inline-block rounded-full bg-primary uppercase leading-normal shadow-md transition duration-150 ease-in-out hover:bg-primary-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-primary-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)]"
                  >
      ...
      {modalEvidencias && (
        ...
        >
          ...
          >
            {evidencesUrl?.firmas &&
              evidencesUrl.firmas.map((archivo, index) => (
                <div
                  key={index}
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      descargarArchivo(archivo.url, archivo.nombrefirma)
                    }
                  >
            ...
            {evidencesUrl?.fotos &&
              evidencesUrl.fotos.map((archivo, index) => (
                <div
                  key={index}
                  className="relative w-full sm:w-1/3 p-2 relative box-border mb-4"
                  /* style={{ position: 'relative', width: 'calc(33.33% - 10px)', margin: '5px', boxSizing: 'border-box' }} */
                  ...
                  {archivo.url && (
                    <>
                      ...
                      <button
                        ...
                        onClick={() =>
                          descargarArchivo(archivo.url, archivo.nombrefoto)
                        }
                      >
                        <i className="pi pi-download" />
                      </button>
            ...
            {evidencesUrl?.evidencia &&
              evidencesUrl.evidencia.map((archivo, index) => (
                <div
                  key={index}
                  className="relative w-full sm:w-1/3 p-2 relative box-border mb-4"
                  /* style={{ position: 'relative', width: 'calc(33.33% - 10px)', margin: '5px', boxSizing: 'border-box' }} */
                  ...
                  {archivo.url && (
                    <>
                      <small
                        ...
                        /* style={{ position: 'absolute', bottom: 0, left: 0, zIndex: 1, backgroundColor: '#010a1c', width: '100%', height: '55px', paddingRight: 12, color: 'white' }} */
                      >
                        {archivo.nombrefoto}
                      </small>
