...
export const CatalogosPage = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch(`${api}/obtener/catalogo/campos/proyecto`, {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch(`${api}/obtener/catalogo/datos/proyecto/${catalogoProyectoSeleccionado}`, {
            method: 'POST', // o 'PUT', 'DELETE', etc., según el método que debas usar
            headers: {
              'Content-Type': 'application/json', // o el tipo de contenido adecuado
              // otras cabeceras según sea necesario
    ...
    const handleAgregar = () => {
      ...
      if (nuevoValorCatalo.trim() !== '') {
        ...
        setNuevoArregloOpcionesCatalogo(prevState => [...prevState, nuevoValorCatalo]);
  ...
  return (
    <>
        <div className="pb-6">
        <h1 className="pt-2 xl:pt-6 pl-3 xl:pl-20 text-4xl font-black text-[#245A95]">Catálogo</h1>
            <div className='mx-4 xl:mx-20 my-4 px-4 py-2 bg-white rounded-lg overflow-hidden'>
            <h1 className="pt-2 xl:pt-6 pl-3 text-base font-black text-[#245A95]">1. Selecciona el proyecto</h1>
