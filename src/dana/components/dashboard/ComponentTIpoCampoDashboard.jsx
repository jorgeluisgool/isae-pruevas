...
export const ComponentTipoCampoDashboard = ({
  ...
  const subirArchivos = (e) => {
    ...
    setArchivos((archivosAnteriores) => [
      ...archivosAnteriores,
      ...nuevosArchivos,
    ]);
    setArchivosSeleccionados((archivosAnteriores) => [
      ...archivosAnteriores,
      ...nuevosArchivos,
    ]);
  };
      ...
      construirDataColeccion().then((dataColeccionResuelta) => {
        const archivosFiltrados = dataColeccionResuelta.filter((archivo) => {
          return !filesArray.some(
            (file) => file.name === archivo.nombreEvidencia
          );
        });

        const archivosEnFilesArrayNoEnData = filesArray.filter((file) => {
          return !dataColeccionResuelta.some(
            (archivo) => archivo.nombreEvidencia === file.name
          );
        });
  ...
  const [selectedValueCatalogo, setSelectedValueCatalogo] = useState(
    campo.valor
  );
  const [selectedValueCatalogoInput, setSelectedValueCatalogoInput] = useState(
    campo.valor
  );
  ...
  const guardar = async (campo, idInventario) => {
    try {
      ...
      const currentSignatures = Array.isArray(signatures)
        ? [...signatures]
        : [];
  ...
  // Función para abrir la ventana de selxección de archivo
  const handleClick = () => {
  ...
  const currentCatalogo =
    dataProyectoSeleccionado?.catalogos?.[campo?.nombreCampo]?.catalogo || [];
  const defaultOption = selectedValueCatalogoInput || "";

  // Si selectedValueCatalogoInput no está en el arreglo del catálogo, agregarlo
  if (
    selectedValueCatalogoInput &&
    !currentCatalogo.includes(selectedValueCatalogoInput)
  ) {
  ...
  return (
    <>
        ...
        {campo.tipoCampo === "FOTO" && (
          <span className="p-float-label relative">
            ...
            <div className="flex">
              ...
              <span
                className="hover:bg-[#245A95] hover:text-[#245A95] cursor-pointer border border-[#fff] p-2 rounded-md ml-2 text-white hover:border-[#245A95]"
        ...
        {campo.tipoCampo === "FIRMA" && (
          <div className="grid h-30 ">
            ...
            <div className="m-auto mt-3">
              ...
              <span
                className=" border border-gray-300 p-2 rounded-md mr-1"
      ...
      <Dialog
        header="FIRMA"
        visible={modalAbrirFirma}
        style={{ width: "40vw" }}
        onHide={() => setModalAbrirFirma(false)}
      >
        <img className="h-50 w-55 bg-white p-2" src={imageURL} alt="Firma" />
      </Dialog>
