...
export const ModalHistorialRegistros = ({
  modalHistorialAbrirCerrar,
  setModalHistorialAbrirCerrar,
  proyectoSeleccionado,
  handleReset,
}) => {
  const { data: historialCambiosData } = useFetchHistorialCambios(
    proyectoSeleccionado,
    modalHistorialAbrirCerrar
  );
  ...
  const currentRows = historialCambiosDataReversed.slice(
    indexOfFirstRow,
    indexOfLastRow
  );
  ...
  return (
    <>
      <Dialog
        ...
        visible={modalHistorialAbrirCerrar}
        style={{ width: "90vw", height: "40vw" }}
        onHide={() => setModalHistorialAbrirCerrar(false)}
        className="mt-16"
      >
        <table className="min-w-full bg-white rounded-lg overflow-hidden shadow-md mb-6">
          ...
          <tbody className="divide-y divide-gray-200">
            {currentRows.map((historialCambio, index) => (
              <tr key={index} className="hover:bg-[#E2E2E2]">
                <td className="px-6">
                  <div className="flex space-x-4">
                    <div className="text-sm font-medium text-gray-900">
                      {historialCambio.campo.campo}
                    </div>
                  </div>
                </td>
                <td className="px-6">
                  <div className="flex space-x-4">
                    <div className="text-sm font-medium text-gray-900">
                      {historialCambio.usuario.usuario}
                    </div>
                  </div>
                </td>
                <td className="px-6">
